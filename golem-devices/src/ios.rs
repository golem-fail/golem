...
pub fn pick_device_type(
    device_types: &[DeviceTypeInfo],
    want_phone: bool,
) -> Option<&DeviceTypeInfo> {
    device_types.iter().rfind(|dt| {
        if want_phone {
            dt.is_phone
        } else {
...
mod tests {
    ...
    fn pick_runtime_none_returns_latest() {
        ...
        let pick =
            pick_runtime_for_spec(&runtimes, None).expect("SHALL return latest when spec is None");
    ...
    fn make_simctl_json(entries: &[(&str, &str)]) -> String {
    ...
    fn parse_two_ios18_devices_booted_and_shutdown() {
        ...
        assert_eq!(booted.last_booted, Some("2026-03-21T10:00:00Z".to_string()));
    ...
    fn runtime_version_parsing() {
        let result = parse_runtime_version("com.apple.CoreSimulator.SimRuntime.iOS-18-6");
        assert_eq!(result, Some((18, "18.6".to_string())));

        let result = parse_runtime_version("com.apple.CoreSimulator.SimRuntime.iOS-17-0");
        assert_eq!(result, Some((17, "17.0".to_string())));

        let result = parse_runtime_version("com.apple.CoreSimulator.SimRuntime.iOS-16-4");
        assert_eq!(result, Some((16, "16.4".to_string())));

        // Non-iOS runtime should return None
        let result = parse_runtime_version("com.apple.CoreSimulator.SimRuntime.tvOS-18-0");
    ...
    fn empty_devices_returns_empty_vec() {
        let json = make_simctl_json(&[("com.apple.CoreSimulator.SimRuntime.iOS-18-4", "[]")]);
    ...
    fn pick_runtime_minimum_returns_latest() {
        ...
        assert_eq!(
            pick.major, 26,
            "Minimum spec SHALL return latest, not the floor"
        );
    }
    ...
    fn pick_device_type_phone_picks_last_phone() {
        ...
        let pick = pick_device_type(&types, true).expect("SHALL find a phone");
        assert_eq!(
            pick.name, "iPhone 16",
            "rfind SHALL return the latest phone"
        );
    }
    ...
    fn pick_device_type_tablet_picks_last_ipad() {
        ...
        let pick = pick_device_type(&types, false).expect("SHALL find an iPad");
        assert_eq!(
    ...
    fn pick_device_type_tablet_uses_name_not_flag() {
        ...
        let pick = pick_device_type(&types, false).expect("SHALL find an iPad by name");
    ...
    fn runtime_version_non_numeric_major_none() {
        let result = parse_runtime_version("com.apple.CoreSimulator.SimRuntime.iOS-beta-1");
    ...
    fn parse_missing_device_type_defaults_to_phone() {
        let dev = make_device_json("Some Device", "LLLL-1234", "Shutdown", true, None, None);
        let json = make_simctl_json(&[(
            "com.apple.CoreSimulator.SimRuntime.iOS-18-4",
            &format!("[{dev}]"),
        )]);
    ...
    fn parse_runtimes_output_filters_and_sorts() {
        ...
        assert_eq!(
            runtimes.len(),
            3,
            "three available iOS runtimes SHALL be kept"
        );
    ...
    fn parse_runtimes_output_drops_unavailable_and_non_ios() {
        ...
        assert_eq!(
            runtimes.len(),
            1,
            "unavailable and non-iOS runtimes SHALL be dropped"
        );
    ...
    fn parse_runtimes_output_invalid_json_err() {
        ...
        assert!(
            result.is_err(),
            "malformed runtimes JSON SHALL yield an Err"
        );
    }
    ...
    fn parse_device_types_output_flags_phones() {
        ...
        assert!(
            !types[2].is_phone,
            "non-iPhone name SHALL set is_phone=false"
        );
    }
    ...
    fn parse_device_types_output_empty_and_invalid() {
        ...
        assert!(
            empty.is_empty(),
            "empty devicetypes SHALL yield an empty vec"
        );

        let result = parse_device_types_output("{ not valid json ]");
        assert!(
            result.is_err(),
            "malformed devicetypes JSON SHALL yield an Err"
        );
    }
    ...
    fn parse_sets_simulator_field_defaults() {
        ...
        assert_eq!(
            devices.len(),
            1,
            "one input device SHALL yield one DeviceInfo"
        );
        let d = &devices[0];
        ...
        assert_eq!(
            d.platform,
            Platform::Ios,
            "platform SHALL be Ios for simulators"
        );
        assert_eq!(
            d.device_type,
            DeviceType::Phone,
            "iPhone-16 SHALL classify as Phone"
        );
        assert_eq!(
            d.state,
            DeviceState::Booted,
            "\"Booted\" state SHALL map to DeviceState::Booted"
        );
        assert_eq!(
            d.os_major, 18,
            "os_major SHALL be parsed from the iOS-18-4 runtime"
        );
        assert_eq!(
            d.os_version, "18.4",
            "os_version SHALL be the dotted form of the runtime version"
        );
        ...
        assert!(
            d.screen_width.is_none(),
            "screen_width SHALL be unset for sims"
        );
        assert!(
            d.screen_height.is_none(),
            "screen_height SHALL be unset for sims"
        );
        assert!(
            d.screen_scale.is_none(),
            "screen_scale SHALL be unset for sims"
        );
    }
}
