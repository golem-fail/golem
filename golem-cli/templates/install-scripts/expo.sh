...
EXPO_DIR="{{EXPO_DIR}}"           # path to the Expo project (contains app.json)
PM_RUNNER="{{PM_RUNNER}}"         # expo CLI runner: npx expo | yarn expo | pnpm expo | bunx expo
PM_INSTALL="{{PM_INSTALL}}"       # dependency install: npm install | yarn | pnpm install | bun install
IOS_SCHEME="{{IOS_SCHEME}}"       # iOS scheme (Expo names it after the app)
