    ...
    while let Ok(event) = rx.recv().await {
        ...
        match &event.kind {
            ...
            EventKind::FlowFinished {
                ...
                let (ae, aw) = a11y_by_device.remove(&event.device_id.0).unwrap_or((0, 0));
                let a11y = a11y_rollup(ae, aw, use_color);
                if use_color {
                    eprintln!(
                        "{ts}  {dp}{kw} {dur}  {name}  {code_str}{DIM}seed:{seed}{RESET}{a11y}"
                    );
                } else {
            ...
            EventKind::DeviceRecovering { device_id, reason } => {
                ...
                if use_color {
                    eprintln!(
                        "{ts}  {dp}{t} {DIM}recovering{RESET} {device_id} {DIM}— {reason}{RESET}"
                    );
                } else {
...
mod tests {
    ...
    fn a11y_rollup_empty_and_formatted() {
        ...
        assert_eq!(
            a11y_rollup(0, 0, true),
            "",
            "no findings SHALL be empty (color)"
        );
        assert_eq!(
        ...
        assert!(
            colored.starts_with(DIM)
                && colored.ends_with(RESET)
                && colored.contains("a11y: 1 error(s), 2 warning(s)"),
