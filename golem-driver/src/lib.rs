//! Platform driver layer: the [`PlatformDriver`] trait and its
//! implementations translate golem's platform-agnostic actions (tap, type,
//! swipe, launch, screenshot, …) into HTTP calls against the on-device
//! companion agent, on both iOS ([`ios::IosDriver`]) and Android
//! ([`android::AndroidDriver`]).
//!
//! `golem-runner` drives flows purely against `dyn PlatformDriver`, so it
//! never needs to know which platform it's talking to. [`common`] holds the
//! shared companion-HTTP plumbing (`common::CompanionClient`) and hierarchy
//! response-parsing helpers used by both drivers; [`cdp`] and [`webkit`] add
//! WebView DOM enrichment over the Chrome DevTools Protocol (Android) and
//! the WebKit Inspector Protocol (iOS); [`ime`] manages golem's companion
//! Unicode input method (Android); [`commands`] builds host-side
//! `adb`/`simctl` command lines; and [`ios_display`] maps iOS device models
//! to their notch/corner geometry. [`MockPlatformDriver`] ([`mock`])
//! and the debug-only [`stub`] driver stand in for a real device in unit
...
pub use mock::MockPlatformDriver;
