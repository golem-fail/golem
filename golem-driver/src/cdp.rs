...
const DOM_TRAVERSAL_JS: &str = r#"(function(){var dpr=window.devicePixelRatio||1;var nc=0;var t0=performance.now();function t(el){nc++;var r=el.getBoundingClientRect();var al=el.getAttribute('aria-label')||'';var tx='';for(var c of el.childNodes){if(c.nodeType===3&&c.textContent.trim()){tx=c.textContent.trim();break;}}var n={class:el.tagName.toLowerCase(),text:al||tx,contentDescription:al||el.id||'',bounds:{left:Math.round(r.left*dpr),top:Math.round(r.top*dpr),right:Math.round((r.left+r.width)*dpr),bottom:Math.round((r.top+r.height)*dpr)},clickable:el.tagName==='BUTTON'||el.tagName==='A'||el.getAttribute('role')==='button',enabled:!el.disabled,checked:!!el.checked,focused:document.activeElement===el,scrollable:false,selected:false,children:[]};for(var c of el.children){if(c.tagName!=='SCRIPT'&&c.tagName!=='STYLE'){var s=window.getComputedStyle(c);if(s.display!=='none'&&s.visibility!=='hidden'){n.children.push(t(c));}}}return n;}var tree=t(document.body);return JSON.stringify({tree:tree,meta:{elapsed_ms:Math.round(performance.now()-t0),node_count:nc,dpr:dpr,url:location.href}});})()
"#;
