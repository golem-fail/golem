...
## Tauri Android back-button on API 35+ (predictive back)

`device_controls.test::android_back_button` assumes `press button =
"back"` is intercepted by Tauri's `WindowEvent::CloseRequested`
handler (the test app calls `preventDefault()` and increments a
`backCount` counter). Works on API 34. Fails on Pixel 8 Pro API 36:
the OS routes back through the new `OnBackInvokedDispatcher`
(predictive back, default-on for apps targeting API 35+), Tauri's
old `onBackPressed` path never fires, the activity finishes and
the test ends up on the launcher.

Adding `android:enableOnBackInvokedCallback="true"` on `<application>`
didn't help — the WebView still has no `OnBackInvokedCallback`
registered. Real fix needs to land in Tauri (or our manifest /
MainActivity could register one explicitly, but `gen/` is
regenerated by `tauri android build`).

**Files:** `test-app/src-tauri/gen/android/app/src/main/AndroidManifest.xml`,
upstream `tauri::WebViewActivity` (Tauri 2.x Android).
