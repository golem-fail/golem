...
///
/// Alphabetical, and kept honest by
/// `golem_runner::actions::tests::actions_reference_doc_lists_every_action`,
/// which compares this list against the runner's dispatch arms, the browser
/// crate's own dispatch, and `docs/actions-reference.md`. It had drifted 27
/// actions behind before that check existed — every `browse_*`, plus
/// `accept_alert`, `double_tap`, `gesture` and `pinch` — which nothing
/// noticed because `validate_flow` had no caller.
///
/// `load_mixin` is here but is not a dispatch arm: the parser expands it
/// into the referenced mixin's steps before anything runs.
