...
pub async fn execute_teardown(
    ...
    for block in teardown_blocks {
        for step in &block.steps {
            ...
            match execute_step_with_policy(&effective_step, driver, vars, default_timeout_ms).await
            {
...
mod tests {
    ...
    async fn teardown_executes_all_steps() {
        ...
        let result = execute_teardown(&blocks, &driver, &mut vars, DEFAULT_TIMEOUT).await;
    ...
    async fn teardown_explicit_on_fail_error_collects_error_no_propagation() {
        ...
        assert_eq!(
            screenshot_calls.len(),
            2,
            "steps around the error step should still execute"
        );
    }
    ...
    async fn empty_teardown_blocks_produce_empty_result() {
        ...
        // No blocks at all
        let result = execute_teardown(&[], &driver, &mut vars, DEFAULT_TIMEOUT).await;
        assert!(result.warnings.is_empty());
        assert!(result.errors.is_empty());
