...
pub(crate) async fn handle_set_location(step: &Step, driver: &dyn PlatformDriver) -> Result<()> {
    let latitude = step
        ...
        .ok_or_else(|| {
            golem_events::coded(
                golem_events::FailureCode::ParseMissingParam,
                anyhow::anyhow!("set_location action requires 'latitude' param"),
            )
        })?;
    let longitude = step
        .params
        ...
        .ok_or_else(|| {
            golem_events::coded(
                golem_events::FailureCode::ParseMissingParam,
                anyhow::anyhow!("set_location action requires 'longitude' param"),
            )
        })?;
...
pub(crate) async fn handle_press(step: &Step, driver: &dyn PlatformDriver) -> Result<()> {
    let button = step
        ...
        .ok_or_else(|| {
            golem_events::coded(
                golem_events::FailureCode::ParseMissingParam,
                anyhow::anyhow!("press action requires 'button' param"),
            )
        })?;
